...
def map_courses_to_sfia(courses_file: str, sfia_file: str, model: str, ps_name: str, threshold=0.2):
    ...
    for _, course_row in courses_df.iterrows():
        ...
        for _, sfia_row in sfia_df.iterrows():
            ...
            if not visualized:
                ...
                os.makedirs(f"Output/{ps_name}/Mapping", exist_ok=True)
                with pd.ExcelWriter(f"Output/{ps_name}/Mapping/contoh_vektor_perbandingan_{model}.xlsx") as writer:
    ...
    filtered_df.to_excel(f"Output/{ps_name}/Mapping/mapping_cosine_{model}_{ps_name}.xlsx", index=False)

    # Ekspansi SFIA skill level pakai expand_skill_levels
    expanded_df = expand_skill_levels(filtered_df, sfia_df)
    expanded_df.to_excel(f"Output/{ps_name}/Mapping/expanded_mapping_cosine_{model}_{ps_name}.xlsx", index=False)
...
if __name__ == "__main__":
    ...
    for ps in program_studies:
        for model in MODELS:
            ...
            try:
                map_courses_to_sfia(
                    f"Output/{ps}/Ekstraksi/skills_extracted_courses_{ps}.xlsx",
                    f"Output/{ps}/Ekstraksi/skills_extracted_sfia_{ps}.xlsx",
                    model,
                    ps,
                    threshold=0.2
                )
